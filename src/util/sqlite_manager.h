...
class SqliteManager final {
 ...
 public:
  ...
  bool Init(const std::string& home_dir) {
    std::string db_path_ = home_dir + "/data/db";
  ...
  int32_t Execute(
      const std::string& sql, int32_t* affect_rows, std::string* err_msg,
      sqlite3_stmt** stmt,
  ...
  // 2. only execute once
  int32_t ExecuteNonQuery(const std::string& sql, std::string* error_msg,
  ...
  bool InitAdminUser() {
    ...
    std::function<void(sqlite3_stmt * stmt)> bind_callback =
        [](sqlite3_stmt* stmt) {
          ...
          sqlite3_bind_text(stmt, 2, salt.c_str(), Util::kSaltSize,
                            SQLITE_STATIC);
          sqlite3_bind_text(stmt, 3, hashed_password.c_str(),
                            Util::kDerivedKeySize, SQLITE_STATIC);
        };
    auto ret = Execute(sql, &affect_rows, &err_msg, stmt, bind_callback);
    if (ret) {
